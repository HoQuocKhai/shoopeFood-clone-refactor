...
function getHeading(from: RoutePoint, to: RoutePoint) {
  const y =
    Math.sin(((to.longitude - from.longitude) * Math.PI) / 180) *
...
export default function TrackingPage() {
  ...
  const driverPoint = driverLocation
    ...
    : null;
  const nextPoint = tracking?.routePoints.find(
    (point) =>
      driverPoint &&
      Math.hypot(point.latitude - driverPoint.latitude, point.longitude - driverPoint.longitude) >
        0.0005
  );
  const motoHeading =
    driverLocation?.heading || (driverPoint && nextPoint ? getHeading(driverPoint, nextPoint) : 0);
            ...
            {driverLocation ? (
              <>
                ...
                <CircleMarker
                  ...
                  radius={16}
                  pathOptions={{ color: '#00b14f', opacity: 0.2 }}
                />
              </>
            ) : null}
          </MapContainer>
        ...
        <aside className="tracking-card">
          <div className="tracking-card-head">
            <span>
              {isLoading
                ? 'Dang tai'
                : tracking?.order.statusLabel || tracking?.order.statusCode || 'Cho tai xe'}
            </span>
