...
import { getDrivers, updateDriverLocation } from '../api/drivers';
import {
  getOrderTracking,
  getOrders,
  updateOrder,
...
export default function DriverPage() {
  ...
  async function loadDashboard(nextDriverId = Number(driverId)) {
    try {
      ...
      const activeOrders = orderData.filter((order) =>
        activeStatusCodes.has(order.statusCode || '')
      );
  ...
  const selectedDriver = useMemo(
    () => drivers.find((driver) => driver.id === Number(driverId)) || null,
    [driverId, drivers]
  );
  const activeOrder = useMemo(
    () => orders.find((order) => order.id === activeOrderId) || tracking?.order || null,
    [activeOrderId, orders, tracking]
  );
        ...
        if (index >= routePoints.length - 1) {
          ...
          void updateOrderStatus(activeOrder.id, 'COMPLETED').then((completed) => {
            ...
            setTracking((currentTracking) =>
              currentTracking ? { ...currentTracking, order: completed } : currentTracking
            );
    ...
    <section className="driver-page">
      <div className="driver-header">
        <div>
          ...
          <p>
            {selectedDriver
              ? `${selectedDriver.fullName} · ${selectedDriver.licensePlate}`
              : 'Chon tai xe de bat dau'}
          </p>
        </div>
        ...
        <aside className="driver-orders">
          <div className="driver-panel-head">
            ...
            <button
              ...
              onClick={() => void loadDashboard(Number(driverId))}
            >
              Tai lai
            </button>
          </div>
          ...
          <div className="driver-order-list">
            {orders.map((order) => (
              <article
                key={order.id}
                className={`driver-order-card ${order.id === activeOrderId ? 'active' : ''}`}
              >
                <button type="button" onClick={() => setActiveOrderId(order.id)}>
                  <strong>{order.orderCode}</strong>
                  <span>{order.receiverAddress || `Don #${order.id}`}</span>
                  <small>
                    {order.statusLabel || order.statusCode} · {formatPrice(order.totalAmount)} VND
                  </small>
                </button>
                <button
                  type="button"
            ...
            {!isLoading && orders.length === 0 ? (
              <p className="empty-state">Khong co don dang giao.</p>
            ) : null}
          </div>
        </aside>
          ...
          <MapContainer center={mapCenter} zoom={14} scrollWheelZoom>
            ...
            {routeLatLng.length > 0 ? (
              <Polyline
                ...
                pathOptions={{ color: '#00b14f', weight: 6, opacity: 0.76 }}
              />
            ) : null}
            {tracking?.restaurant ? (
              <Marker
                position={[tracking.restaurant.latitude, tracking.restaurant.longitude]}
                icon={makePinIcon('restaurant-pin', 'R')}
              >
        ...
        <aside className="driver-control-panel">
          ...
          <div className="driver-actions-grid">
            <button
              type="button"
              className="button-secondary"
              onClick={() => void handleStatus('PICKING_UP')}
            ...
            <button
              type="button"
              className="button-primary"
              onClick={() => void startDeliveryRun()}
              disabled={!activeOrder || isRunning}
            >
              Chay giao hang
            </button>
            <button
          ...
          <div className="driver-summary">
            ...
            <div>
              <span>So mon</span>
              <strong>
                {activeOrder?.items.reduce((total, item) => total + item.quantity, 0) || 0}
              </strong>
            </div>
          </div>
        </aside>
