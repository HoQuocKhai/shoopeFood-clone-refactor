...
import {
  ...
  getCategories,
  updateCategory,
} from '../../menu/api/categories';
import { createFood, deleteFood, getFoods, updateFood } from '../../menu/api/foods';
import { getRestaurants } from '../../restaurants/api/restaurants';
    ...
    fields: [
      {
        ...
        label: 'Trang thai',
        type: 'select',
        options: ['PENDING', 'CONFIRMED', 'PICKING_UP', 'DELIVERING', 'COMPLETED', 'CANCELLED'],
      },
...
function getInitialForm(fields: FieldConfig[], record?: AdminRecord | null) {
  return fields.reduce<Record<string, FieldValue>>((values, field) => {
    ...
    values[field.key] =
      (sourceValue as FieldValue | undefined) ??
      field.defaultValue ??
...
function AdminResourcePanel({ config }: AdminResourcePanelProps) {
  ...
  const { mutateAsync: updateRecord, mutation: updateMutation } = useUpdate<
    AdminRecord,
    HttpError,
    Record<string, unknown>
  >({
  ...
  const { mutateAsync: deleteRecord, mutation: deleteMutation } = useDelete<
    AdminRecord,
    HttpError
  >();
    ...
    <div className="admin-workspace">
      <section className="admin-panel">
        ...
        {feedback ? (
          <p
            className={
              feedback.includes('that bai') || feedback.includes('Khong')
                ? 'app-feedback error'
                : 'restaurant-feedback success'
            }
          >
            {feedback}
          </p>
        ...
        <div className="admin-table-wrap">
          ...
          {!query.isLoading && records.length === 0 ? (
            <p className="empty-state">Chua co du lieu.</p>
          ) : null}
        </div>
      </section>
      ...
      <aside className="admin-form-panel">
        <div className="driver-control-head">
          ...
          <p>
            {canCreate || editingRecord
              ? 'Nhap thong tin va luu vao backend.'
              : 'Resource nay chi cap nhat ban ghi co san.'}
          </p>
        </div>
        ...
        <form className="admin-form" onSubmit={handleSubmit}>
          {config.fields.map((field) => (
            <label
              key={field.key}
              className={field.type === 'checkbox' ? 'restaurant-checkbox' : 'restaurant-field'}
            >
              {field.type === 'checkbox' ? (
                <>
                  <input
          ...
          <div className="restaurant-form-actions">
            <button
              ...
              disabled={isSaving || (!canCreate && !editingRecord)}
            >
...
function MenuManagerPanel() {
  ...
  const stats = useMemo(() => {
    const available = foods.filter(
      (food) => food.isAvailable && Number(food.currentQuantity || 0) > 0
    ).length;
    ...
    <div className="menu-manager">
      <section className="menu-manager-hero">
        <div>
          ...
          <p>
            CRUD danh muc, CRUD mon an va kiem tra API loc menu theo restaurant, category, ten mon,
            trang thai ban.
          </p>
        </div>
      ...
      <div className="menu-manager-layout">
        <section className="admin-panel menu-list-panel">
          ...
          {!isLoading && foods.length === 0 ? (
            <p className="empty-state">Khong co mon phu hop bo loc.</p>
          ) : null}
        </section>
        ...
        <aside className="menu-editor-stack">
            ...
            <form className="admin-form" onSubmit={(event) => void handleFoodSubmit(event)}>
              ...
              <label className="restaurant-field">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setFoodForm((current) => ({ ...current, categoryId: event.target.value }))
                  }
                >
              ...
              <label className="restaurant-checkbox">
                <input
                  ...
                  onChange={(event) =>
                    setFoodForm((current) => ({ ...current, isAvailable: event.target.checked }))
                  }
                />
                <span>Dang ban</span>
              </label>
          ...
          <section className="admin-form-panel">
            <div className="driver-control-head">
              <span>
                {categoryForm.id ? `Edit danh muc #${categoryForm.id}` : 'Create category'}
              </span>
              <h2>Danh muc</h2>
    ...
    <section className="admin-page">
      <div className="admin-header">
        <div>
          ...
          <p>
            {user
              ? `${user.fullName || user.phone} dang dang nhap voi role ${user.role}`
              : 'Quan ly du lieu he thong.'}
          </p>
        </div>
