import { httpDelete, httpGet, httpPost, httpPut, httpPatch } from '../../../services/api/http';
import type {
  ApiResponse,
  Restaurant,
...
export async function updateRestaurant(id: number, payload: RestaurantUpdateInput) {
  const response = await httpPut<
    ApiResponse<{ restaurant: Restaurant; changeRequest: RestaurantChangeRequest | null }>
  >(`/api/restaurants/${id}`, payload);
  return response.data;
}
...
export async function patchRestaurantStatus(id: number, isOpen: boolean) {
  const response = await httpPatch<ApiResponse<Restaurant>>(`/api/restaurants/${id}/status`, {
    isOpen,
  });
  return response.data;
}
...
export async function approveRestaurant(id: number, approvedBy?: number) {
  const response = await httpPatch<ApiResponse<Restaurant>>(
    `/api/restaurants/admin/${id}/approve`,
    {
      approvedBy,
    }
  );
...
export async function getRestaurantChangeRequests(status?: string) {
  const response = await httpGet<ApiResponse<RestaurantChangeRequest[]>>(
    '/api/restaurants/admin/change-requests',
    {
      query: status ? { status } : {},
    }
  );
