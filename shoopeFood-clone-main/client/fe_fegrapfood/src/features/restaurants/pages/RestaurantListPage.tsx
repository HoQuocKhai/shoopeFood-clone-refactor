...
export default function RestaurantListPage() {
  ...
  async function loadData() {
    try {
      ...
      if (activeTab === 'list') {
        const items = isAdmin
          ? await getAllRestaurantsForAdmin()
          : isMerchant
            ...
            {restaurants.map((restaurant) => (
              <article key={restaurant.id} className="restaurant-manage-card">
                <div className="restaurant-manage-top">
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    ...
                    {restaurant.approvalStatus && (
                      <span
                        className={`approval-tag approval-${restaurant.approvalStatus.toLowerCase()}`}
                      >
                ...
                <p className="restaurant-location">
                  Lat: {formatCoordinate(restaurant.latitude)} | Lng:{' '}
                  {formatCoordinate(restaurant.longitude)}
                </p>

                {restaurant.isOpenToday === false && (
                  <p className="restaurant-temp-closed">
                    ⚠️ Đóng tạm: {restaurant.temporaryClosedReason} (đến{' '}
                    {formatDateTime(restaurant.temporaryClosedUntil)})
                  </p>
                )}
            ...
            {changeRequests.map((request) => (
              <div key={request.id} className="change-request-card">
                <div className="change-request-header">
                  <h3>
                    Yêu cầu #{request.id} - Nhà hàng #{request.restaurantId}
                  </h3>
                  <span className="approval-tag approval-pending">Chờ duyệt</span>
                </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTodayStatusData({ ...todayStatusData, reason: e.target.value })
                  }
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTodayStatusData({ ...todayStatusData, until: e.target.value })
                  }
                  style={{ width: '100%', padding: '0.5rem', marginTop: '0.25rem' }}
                />
              </label>
